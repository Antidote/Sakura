...
{
  const sf::Uint32* a = reinterpret_cast<const sf::Uint32*>(info.text);
  const sf::Uint32* b = reinterpret_cast<const sf::Uint32*>(vendorString);
...
# else // _MSC_VER < 1400
  sf::Uint32 cpuid_in = in;
  sf::Uint32* cpuid_out = out->i;
...
void x86CpuDetect(X86CpuInfo* out)
{
  ...
  memset(reinterpret_cast<sf::Uint8*>(out) + sizeof(sf::Uint32),
    0, sizeof(CpuInfo) - sizeof(sf::Uint32));
  ...
  sf::Uint32 exIds = regs.eax;
  if (exIds > 0x80000004) exIds = 0x80000004;

  sf::Uint32* brand = reinterpret_cast<sf::Uint32*>(out->_brandString);
