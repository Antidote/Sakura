    ...
    if (m_state == Opened || m_state == Opening)
    {
        if (unicode > 0x1f && (unicode != 0x7e && unicode != 0x7f))
        {
            if (m_overwrite && (m_commandString.getSize() > 0 && m_cursorPosition < (int)m_commandString.getSize()))
            {
                ...
                if (m_cursorPosition < ((int)currentMaxLen() - 1))
                {
                    m_cursorPosition++;
            ...
            if (m_commandString.getSize() >= currentMaxLen())
                return;
...
void Console::handleInput(sf::Keyboard::Key code, bool alt, bool control, bool shift, bool system)
{
    ...
    switch(code)
    {
        ...
        case sf::Keyboard::Right:
        {
            ...
            m_cursorPosition++;
            m_cursorX += (m_drawText.getFont()->getGlyph(m_drawText.getString()[m_cursorPosition - 1], m_drawText.getCharacterSize(), false).advance);            m_showCursor = true;
            m_cursorTime = sf::seconds(0.f);
        }
        break;
        ...
        case sf::Keyboard::Insert:
        {
            ...
            if (m_overwrite && m_cursorPosition >= (int)m_commandString.getSize())
            {
                m_cursorPosition--;
                m_cursorX -= (m_drawText.getFont()->getGlyph(m_drawText.getString()[m_cursorPosition + 1], m_drawText.getCharacterSize(), false).advance);
            }
            else if (m_cursorPosition >= (int)m_commandString.getSize() - 1)
            {
                m_cursorPosition++;
                m_cursorX += (m_drawText.getFont()->getGlyph(m_drawText.getString()[m_cursorPosition - 1], m_drawText.getCharacterSize(), false).advance);
            }
...
void Console::print(Console::Level level, const std::string& fmt, ...)
{
    ...
    for (std::string string : entries)
    {
        ...
        if (string.size() >= currentMaxLen())
        {
            bool hasSpaces = true;
            std::vector<std::string> parts = zelda::utility::split(string, ' ');
            if (parts.size() <= 1)
            {
                parts.clear();
                hasSpaces = false;
...
{
    m_maxLines = std::abs(m_conHeight / (m_drawText.getCharacterSize())) - 2;
}

void Console::recalcMaxLen()
{
    int maxLen = 0;
    int width = 0;
    while (width < com_windowWidth->toInteger())
    {
        width += m_drawText.getFont()->getGlyph((Uint32)sf::String('@')[0], m_drawText.getCharacterSize(), false).advance;
        maxLen++;
    }

    m_currentMaxLen = std::min((int)MAX_LEN, maxLen - 2);
}
...
sf::Uint32 Console::currentMaxLen()
{
    return m_currentMaxLen;
}
